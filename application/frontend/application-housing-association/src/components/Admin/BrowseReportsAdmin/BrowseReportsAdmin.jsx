...
import "./BrowseReportsAdminStyle.css";
import { getUserRole } from "../../../getUserRole";

export const BrowseReportsAdmin = () => {
  ...
  useEffect(() => {
    const fetchHousingAssociation = async () => {
      try {
        const response = await fetch(`${config.BASE_URL}/what-is-my-housing-association`, {
          method: "GET",
          headers: {
    ...
    if (token && (role === "administrator" || role === "resident" || role === "worker")) {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        let endpoint = "";
        if (housingAssociation && housingAssociation.housing_association_id) {
          endpoint = `${config.BASE_URL}/get-reports/${housingAssociation.housing_association_id}`;
        } else {
          endpoint = `${config.BASE_URL}/get-all-reports`;
        }
  ...
  const ReportCard = ({ report }) => {
    ...
    return (
      <div className="browse-reports-admin-card">
        <h3>{report.title}</h3>
        <p>
    ...
    <>
      <div className="start-window-admin-container">
        ...
        <div className="start-window-admin-content">
          <div className="browse-reports-section-admin">
            <div className="browse-reports-edit-section">
              <h2>Przeglądanie zgłoszeń</h2>
              <div className="browse-reports-grid-admin">
                <div className="browse-reports-grid-admin-container">
            ...
            <div className="pagination-browse-reports-admin-grid">
              {Array.from({ length: totalPages }, (_, idx) => idx + 1).map((number) => (
                <button
                  id="number-of-pagination-browse-reports-grid-admin"
                  key={number}
